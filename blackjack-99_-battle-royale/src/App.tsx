...
export default function App() {
  ...
  const startRound = (currentPlayers: Player[], currentDeck: Card[]) => {
    ...
    const resettingPlayers = currentPlayers.map((p) => {
    ...
    const updatedPlayers = resettingPlayers.map((p) => {
      if (p.status === 'eliminated') return p;
      ...
      prev
        ? {
            ...
            dealer: {
              ...
              score: calculateScore([dealerCard1, dealerCard2]),
              hidden: true,
            },
            deck,
            phase: 'player-turn',
  ...
  const handleHit = useCallback(() => {
    ...
    updatedPlayers[playerIndex] = {
      ...player,
      hand: newHand,
      status: newStatus as PlayerStatus,
    };
      ...
      setGameState((prev) => {
        ...
        if (prev.timeLeft <= 1) {
          const finalPlayers = prev.players.map((p) =>
            p.status === 'playing' ? { ...p, status: 'standing' as PlayerStatus } : p
          );
            ...
            if (Math.random() > 0.7) {
              ...
              if (action === 'hit' && newDeck.length > 0) {
                ...
                return {
                  ...p,
                  hand,
                  status: s > 21 ? 'busted' : ('playing' as PlayerStatus),
                };
              }

              return { ...p, status: 'standing' as PlayerStatus };
            }
          }
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mb-12">
                  ...
                  <p className="text-gray-400 max-w-md mx-auto text-lg font-medium tracking-tight">
                    The high-stakes survival battle royale. <span className="text-white">99 players</span> enter, only{' '}
                    <span className="text-stake-green">one</span> survives.
                  </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
                  ...
                  >
                    ...
                    <div className="flex flex-col items-start relative z-10">
                      <div className="bg-stake-green/10 text-stake-green px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 border border-stake-green/20">
                        Solo Entry
                      </div>
                      <h3 className="text-3xl font-black mb-3 italic tracking-tight">Offline Arena</h3>
                      <p className="text-gray-400 text-sm text-left max-w-[200px]">
                        Hunt down 98 bots in a high-speed elimination match.
                      </p>
                    </div>
                  </motion.button>
                  ...
                  >
                    ...
                    <div className="flex flex-col items-start relative z-10">
                      <div className="bg-indigo-accent/10 text-indigo-accent px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 border border-indigo-accent/20">
                        Multiplayer
                      </div>
                      <h3 className="text-3xl font-black mb-3 italic tracking-tight">Online Royale</h3>
                      <p className="text-gray-400 text-sm text-left max-w-[200px]">
                        The ultimate test. Play against real survivors for the crown.
                      </p>
                    </div>
                  </motion.button>
                </div>
              ...
              <div className="flex-1 relative flex flex-col p-4 md:p-8 xl:px-80">
                ...
                <SidebarPlayerList
                  players={gameState.players
                    .filter((p) => p.id !== 'player-1')
                    .slice(0, Math.ceil((gameState.players.length - 1) / 2))}
                  side="left"
                />
                ...
                <SidebarPlayerList
                  players={gameState.players
                    .filter((p) => p.id !== 'player-1')
                    .slice(Math.ceil((gameState.players.length - 1) / 2))}
                  side="right"
                />
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-4 pt-2">
                  <div className="relative group">
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      ...
                      <div className="text-[10px] uppercase font-black text-stake-green tracking-[0.4em] text-center mb-2 drop-shadow-[0_0_8px_rgba(34,197,94,0.4)]">
                        The Dealer
                      </div>

                      <div className="flex justify-center space-x-2 min-h-[100px] scale-90">
                        {gameState.dealer.hand.map((card, i) => (
                          <CardComponent key={i} card={card} hidden={gameState.dealer.hidden && i === 1} index={i} />
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-center relative py-2 w-full max-w-4xl">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="bg-black/40 px-4 py-2 rounded-2xl border border-white/5 flex items-center space-x-4 shadow-inner">
                        ...
                        <div>
                          <div className="text-[9px] uppercase font-black text-gray-500 tracking-[0.2em]">
                            Your Value
                          </div>
                          <div className="text-2xl font-black text-white tabular-nums">
                            {calculateScore(gameState.players[0].hand)}
                          </div>
                        </div>
                      </div>
                      ...
                      {gameState.phase === 'player-turn' && !currentPlayer?.isBot && (
                        <div className="flex items-center space-x-4">
                          ...
                          >
                            <span className="text-[9px] font-black opacity-60 group-hover:opacity-100 uppercase tracking-[0.1em]">
                              Take Card
                            </span>
                      ...
                      <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 w-80 bg-white/5 p-4 rounded-2xl border border-white/5">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={cn(
                              ...
                              gameState.timeLeft < 5 ? 'text-stake-red animate-pulse' : 'text-cyan-accent'
                            )}
                          >
                        ...
                        <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                          <motion.div
                            ...
                            className={cn(
                              'h-full bg-gradient-to-r from-blue-400 to-cyan-300 transition-colors',
                              gameState.timeLeft < 5 && 'from-stake-red to-red-400'
                            )}
                          />
                        </div>
                      </div>
                  ...
                  {eliminatedPlayer && (
                    ...
                    >
                      <div className="text-center">
                        ...
                        <h2 className="text-7xl font-black italic uppercase tracking-tighter text-white mb-2">
                          ELIMINATED
                        </h2>

                        <p className="text-text-secondary text-xl mb-12">
                          You failed to survive Round {gameState.round}.
                        </p>
                  ...
                  {gameState.phase === 'game-over' && gameState.winnerId && (
                    ...
                    >
                      <div className="text-center max-w-lg">
                        ...
                        <h2 className="text-8xl font-black italic uppercase tracking-tighter text-white leading-none mb-4">
                          CHAMPION
                        </h2>

                        <p className="text-stake-green text-2xl font-bold uppercase tracking-[0.3em] mb-12">
                          Sole Survivor
                        </p>
