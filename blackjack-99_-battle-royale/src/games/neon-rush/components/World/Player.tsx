...
export const Player: React.FC = () => {
  ...
  const { armorMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
      const armorColor = isImmortalityActive ? '#ffd700' : '#ff5500'; // Vibrant Orange
      const glowColor = isImmortalityActive ? '#ffffff' : '#ffffff'; // White backpack/accents
      const clothColor = '#3b82f6'; // Blue jeans/pants
      
      return {
          armorMaterial: new THREE.MeshStandardMaterial({ color: armorColor, roughness: 0.8, metalness: 0.1 }),
          jointMaterial: new THREE.MeshStandardMaterial({ color: clothColor, roughness: 0.9, metalness: 0.0 }),
          glowMaterial: new THREE.MeshStandardMaterial({ color: glowColor, roughness: 0.5, metalness: 0.1 }),
