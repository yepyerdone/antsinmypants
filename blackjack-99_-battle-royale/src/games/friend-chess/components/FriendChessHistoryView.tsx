...
import type { LobbyData, MoveRecord } from '../types';
...
export default function FriendChessHistoryView({ onBack, initialGame }: FriendChessHistoryViewProps) {
  ...
  useEffect(() => {
    async function fetchHistory() {
      ...
      try {
        ...
        const allGames = [...snapW.docs, ...snapB.docs].map((d) => ({ id: d.id, ...d.data() } as LobbyData));
  ...
  useEffect(() => {
    async function fetchMoves() {
      ...
      const movesRef = collection(db, FC_COLLECTIONS.lobbies, selectedGame.id, 'moves');
      const q = query(movesRef, orderBy('index', 'asc'));
      const snap = await getDocs(q);

      if (!snap.empty) {
        const moveData = snap.docs.map((d) => d.data() as MoveRecord);
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center bg-fc-bg-dark">
        <div className="w-8 h-8 border-2 border-fc-gold border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
                ...
                <div className="grid gap-4">
                  {games.map((g) => (
                    <div
                      ...
                      className="bg-fc-bg-panel border border-fc-border-dim p-6 rounded-xl hover:border-fc-gold transition-all cursor-pointer group flex items-center justify-between"
                    >
                      <div className="flex items-center gap-6">
                        <div className="w-12 h-12 bg-[#141414] rounded-lg flex items-center justify-center text-fc-gold border border-[#222]">
                          <Trophy size={20} />
                        </div>
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <h3 className="font-bold text-lg uppercase tracking-wider">{g.code}</h3>
                            <span className="text-[10px] text-[#444] font-mono tracking-widest">
                              {g.updatedAt && typeof g.updatedAt === 'object' && g.updatedAt !== null && 'seconds' in g.updatedAt
                                ? new Date((g.updatedAt as { seconds: number }).seconds * 1000).toLocaleDateString()
                                : ''}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-xs text-[#666] tracking-wide">
                            <span className={g.winner === 'White' ? 'text-fc-gold' : ''}>{g.whiteName} (W)</span>
                            <span className="text-[#333]">vs</span>
                            <span className={g.winner === 'Black' ? 'text-fc-gold' : ''}>{g.blackName} (B)</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-2">
                        <span className="text-[10px] uppercase font-bold tracking-[0.2em] border border-fc-gold/20 text-fc-gold px-2 py-1 rounded">
                          {g.winner === 'Draw' ? 'Draw' : `${g.winner} Won`}
                        </span>
                        <div className="text-[9px] text-[#444] uppercase tracking-widest font-bold">GAME FINISHED</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            >
              <div className="flex-1 p-4 lg:p-12 flex items-center justify-center bg-[#080808]">
                <div className="w-full max-w-[min(85vh,750px)] aspect-square rounded-xl overflow-hidden shadow-2xl border-[12px] border-[#1A1A1A]">
                  <Chessboard
                    options={{
                      id: `FriendChessHistoryBoard-${selectedGame.id}`,
                      position: viewerGame.fen(),
                      boardOrientation: selectedGame.playerB === user?.uid ? 'black' : 'white',
                      darkSquareStyle: {
                        backgroundColor: (BOARD_THEMES.find((t) => t.id === selectedGame.theme) || DEFAULT_THEME).dark,
                      },
                      lightSquareStyle: {
                        backgroundColor: (BOARD_THEMES.find((t) => t.id === selectedGame.theme) || DEFAULT_THEME).light,
                      },
              ...
              <div className="w-full lg:w-[400px] border-l border-fc-border-dim bg-fc-bg-panel p-8 flex flex-col shrink-0">
                <div className="mb-8">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <p className="text-fc-gold text-[10px] uppercase font-bold tracking-[0.2em] mb-2">Game Review</p>
                    ...
                    {moves.map((m, idx) => (
                      <button
                        ...
                        className={`p-2 rounded text-left transition-all ${
                          moveIndex === idx ? 'bg-fc-gold text-black font-bold' : 'text-[#666] hover:bg-[#111]'
                        }`}
                      >
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-4 bg-[#111] border border-[#222] text-white rounded-xl flex items-center justify-center disabled:opacity-30 hover:bg-[#1A1A1A] transition-all"
                    >
                      <ChevronLeft />
                    </button>
                    <button
                  ...
                  <button
                    type="button"
                    onClick={resetView}
                    className="w-full py-3 text-[10px] uppercase font-bold tracking-[0.3em] text-[#444] hover:text-white transition-colors flex items-center justify-center gap-2"
                  >
