...
export default function FriendChessSettings({ onBack }: FriendChessSettingsProps) {
  ...
  useEffect(() => {
    async function fetchUserSettings() {
      if (!user) return;
      try {
        const userDoc = await getDoc(doc(db, FC_COLLECTIONS.users, user.uid));
        if (userDoc.exists()) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
      <header className="h-16 border-b border-[#222] flex items-center px-8 bg-[#0a0a0a] shrink-0">
        <button type="button" onClick={onBack} className="p-2 hover:bg-[#222] rounded-full transition-colors mr-4">
          <ArrowLeft size={20} />
        </button>
      ...
      <main className="flex-1 overflow-auto p-6 lg:p-12">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-12">
            <Palette className="text-fc-gold" size={24} />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="space-y-6">
              <h3 className="text-xs font-bold uppercase tracking-widest text-fc-gold mb-4">Select Theme</h3>
                  ...
                  >
                    ...
                    <p
                      className={`text-[10px] font-bold uppercase tracking-wide ${
                        selectedThemeId === theme.id ? 'text-white' : 'text-[#888]'
                      }`}
                    >
                      {theme.name}
                    </p>
